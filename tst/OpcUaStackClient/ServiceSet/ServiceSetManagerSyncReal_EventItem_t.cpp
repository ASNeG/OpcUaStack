...
BOOST_AUTO_TEST_SUITE(ServiceSetManagerSyncReal_EventItem)
...
BOOST_AUTO_TEST_CASE(ServiceSetManagerSyncReal_EventItem)
{
	std::cout << "ServiceSetManagerSyncReal_EventItem_t" << std::endl;
}

BOOST_FIXTURE_TEST_CASE(ServiceSetManagerSyncReal_EventItem_event, GValueFixture)
{
	...
	subscriptionServiceConfig.eventNotificationHandler_ =
		[this](const OpcUaStackCore::EventFieldList::SPtr& eventFieldList) {
			...
			if (eventFieldList->clientHandle() != 218) return;
