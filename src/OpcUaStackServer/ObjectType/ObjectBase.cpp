...
namespace OpcUaStackServer
{
	...
	, namespaceName_("")
	, writeCallback_(boost::bind(&ObjectBase::writeValue, this, _1))
	{
	}
	...
	void
	ObjectBase::applicationServiceIf(ApplicationServiceIf* applicationServiceIf)
	{
	...
	ObjectBase::linkInstanceWithModel(const OpcUaNodeId& nodeId)
	{
		if (applicationServiceIf_ == nullptr) {
			Log(Error, "application service interface error in class ObjectBase");
			return false;
		}
	...
	bool
	ObjectBase::createAndLinkInstanceWithModel(
	...
	bool
	ObjectBase::getNamespaceIndexFromNamespaceName(const std::string& namespaceName, uint16_t& namespaceIndex)
	{
		if (applicationServiceIf_ == nullptr) {
			Log(Error, "application service interface error in class ObjectBase");
			return false;
	...
	void
	ObjectBase::variableTypeNamespace(const std::string& namespaceName)
	{
		namespaceName_ = namespaceName;
	}

	void
	ObjectBase::variableType(const OpcUaNodeId& variableType)
	{
		variableType_ = variableType;
	}

	OpcUaNodeId&
	ObjectBase::variableType(void)
	{
	...
	// ------------------------------------------------------------------------
	bool
	ObjectBase::getNodeIdFromResponse(BrowsePathToNodeIdResponse::SPtr& res, uint32_t idx, OpcUaNodeId::SPtr& nodeId)
	{
	...
	bool
	ObjectBase::getRefFromResponse(GetNodeReferenceResponse::SPtr& res, uint32_t idx, BaseNodeClass::WPtr& ref)
	{
	...
	void
	ObjectBase::writeValue(ApplicationWriteContext* applicationWriteContext)
	{
